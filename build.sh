#!/usr/bin/env bash
#
# build.sh — build (and optionally install) the maze-snapshots package.
#
#   ./build.sh              build  ->  ./maze-snapshots-<ver>.pkg.tar.zst
#   ./build.sh --install    build, then install it with pacman (needs root)
#   ./build.sh --repo DIR   build, then add it to the pacman repo DIR
...
echo ">> building maze-snapshots with makepkg…"
makepkg -f --nodeps

PKGFILE=$(ls -t maze-snapshots-*.pkg.tar.* 2>/dev/null | head -1 || true)
